...
function animateRoute(tour) {
  ...
  function tick() {
    if (step <= tour.length) {
      ...
      solving = false;
      setStatus('Rute selesai. Klik peta untuk menambah Titik Lokasi atau ubah mode perjalanan.', 'green');
    }
  }
  tick();
}
...
function addCity(lat, lng) {
  ...
  marker.on('dragend', function (ev) {
    ...
    document.getElementById('state-badge').textContent = cities.length + ' Titik Lokasi';
    document.getElementById('state-badge').className   = 'badge badge-blue';
    setStatus('Titik Lokasi dipindahkan. Klik Solve untuk memperbarui rute.', 'amber');
  });
...
function clearAll() {
  ...
  if (routeLine) { map.removeLayer(routeLine); routeLine = null; }
  ['m-dist','m-travel'].forEach(id =>
  document.getElementById(id).textContent = '—');
  document.getElementById('state-badge').textContent = '0 Titik Lokasi';
...
function deleteCity(cityIdx) {
  ...
  map.removeLayer(markers[cityIdx]);

  // Hapus dari array
  cities.splice(cityIdx, 1);
  markers.splice(cityIdx, 1);

  // Reset rute
  if (routeLine) { map.removeLayer(routeLine); routeLine = null; }
  lastTour = null; lastD = null;

  // Re-assign icon & popup semua marker (karena index bergeser)
  markers.forEach((m, i) => {
    m.setIcon(makeIcon(i));
    m.setPopupContent(makeCityPopup(i, null, null));

    // Update event dragend agar pakai index baru
    m.off('dragend');
  ...
  // Reset metric footer
  ['m-dist','m-travel'].forEach(id => document.getElementById(id).textContent = '—');
  document.getElementById('state-badge').textContent = cities.length > 0
    ? cities.length + ' Titik Lokasi' : '0 Titik Lokasi';
  document.getElementById('state-badge').className = cities.length > 0
    ? 'badge badge-blue' : 'badge badge-amber';

  updateUI();
  setStatus(
    cities.length >= 2
      ? `Titik dihapus. ${cities.length} titik tersisa. Klik Solve untuk memperbarui rute.`
      : cities.length === 1
        ? 'Titik dihapus. Tambahkan 1 titik lagi untuk Solve.'
        : 'Semua titik dihapus.',
    cities.length >= 2 ? 'amber' : 'blue'
  );
}
/* ═══════════════════════════════════════════════════
   POPUP BUILDER
...
function makeCityPopup(cityIdx, seq, distToNext) {
  ...
  <div class="popup-header">
    <b>Titik Lokasi ${cityIdx + 1}</b>
    <button class="btn-delete-popup" onclick="deleteCity(${cityIdx})">🗑️ Hapus</button>
  </div>
...
function updateUI() {
  ...
  if (n > 0) {
    document.getElementById('state-badge').textContent = n + ' Titik Lokasi';
    document.getElementById('state-badge').className   = 'badge badge-blue';
  }
  renderCityList();
}
  ...
  cities.forEach((c, i) => {
    ...
    div.innerHTML =
      `<div class="city-dot" style="background:${COLORS[i % COLORS.length]}"></div>
       <span class="city-name">Titik Lokasi ${i + 1}</span>
       ${seqLabel}${distLabel}
       <button class="btn-delete-list" onclick="deleteCity(${i})" title="Hapus titik ini">✕</button>`;
...
map.on('click', function (e) {
  ...
  setStatus(
    `Titik Lokasi ${cities.length} ditambahkan. ${cities.length >= 2
      ? 'Klik Solve untuk mencari rute terpendek.'
      : 'Tambahkan satu Titik Lokasi lagi untuk mengaktifkan Solve.'}`,
...
document.getElementById('btn-random').addEventListener('click', function () {
  ...
  setStatus(`${n} Titik Lokasi acak dihasilkan. Klik Solve untuk mencari rute.`, 'green');
});
...
document.getElementById('btn-clear').addEventListener('click', function () {
  if (solving) return;
  clearAll();
  setStatus('Semua Titik Lokasi dihapus. Klik peta untuk mulai dari awal.', 'blue');
});
